...
namespace SPA {
	...
	void CLLMPanel::DisplaySamplerSettings() { // TODO: About modals for each setting
		...
		*/
		if (ImGui::SliderFloat("Temperature", &m_temp, 0.0f, 1.0f, "%.2f")) {
			m_manager.GetLLMEngine()->SetTemperature(m_temp);
		}
		/*
		...
		*/
		if (ImGui::SliderFloat("Top-P", &m_top_p, 0.0f, 1.0f, "%.2f")) {
			m_manager.GetLLMEngine()->SetTopP(m_top_p);
		}
		/*
		Min-P (m_min_p)
		...
		*/
		if (ImGui::SliderFloat("Min-P", &m_min_p, 0.0f, 1.0f, "%.2f")) {
			m_manager.GetLLMEngine()->SetMinP(m_min_p);
		}
		/*
		Top-K (m_top_k)
		...
		*/
		if (ImGui::SliderInt("Top K", &m_top_k, 0, 200)) {
			m_manager.GetLLMEngine()->SetTopK(m_top_k);
		}
		...
		*/
		...
		if (ImGui::InputInt("Seed (-1 = random)", &seed_input)) {
			...
			m_manager.GetLLMEngine()->SetSeed(m_seed);
		}
	...
	void CLLMPanel::DisplayContextSettings() {
		...
		if (send_input_text && m_input_buffer[0] != '\0') {
			...
			VoxBox::SInferenceResult result = m_manager.GetLLMEngine()->Query(prompt);
			if (result.Success()) {
